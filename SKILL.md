...
| `references/chip_distribution_analysis.md` | 询问 主力动向、筹码分布、吸筹/派发、启动前筹码集中度；**同板块多选一时必须加载** | 筹码单峰密集、主力洗盘与出货判定、筹码集中度代理指标(横盘20日+振幅<15%+换手<3%+量比<0.7)、同板块截面比较Tiebreaker |
...
| `references/signal_classification.md` | 用户询问"明天怎么办"、"该不该卖"、"能不能买"等批量决策，或询问精确入场/卖出信号判定、回调结束确认、诱空陷阱、中继突破、恐慌反包等 | 三信号框架(❌💰🟡)、E-1同价位多次受阻(≥3次❌+接近预警)、E0高位警告(20日窗口,≥5次❌/≥3次🟡)、卖出9条件(S1-S9)、入场8信号(E1-E8)、天量以换手率>10%为准 |
...
> 如果当前时间为工作日的开盘时间（**9:00 - 16:00 之间**），由于当天交易尚未收盘或刚刚收盘，脚本和人工点评在分析当天成交量时，**必须结合 A 股日内分时成交规律（U型曲线）非线性推算全天成交量**（例如：开盘前30分钟占比约30%，前1小时约45%，上午盘约65%）。**绝对禁止**进行简单等比放大（防止开盘段成交量过大时被大幅高估），也**绝对禁止**直接用盘中未完成的成交量与历史整天成交量对比说出“缩量”等不专业的判定。
...
| **定时扫盘模式** | 系统定时任务（cron job）自动触发，批量扫描多只股票 | 对每只股票运行 `python3 scripts/analyze_trend.py <代码>`，**直接信任脚本输出的 `signal.status` 字段作为最终信号**（💰入场 / 🟡持有 / ❌卖出），附带关键数据（价格、偏离%、止损位、一句话理由），按 ❌→💰→🟡 排序。 |
